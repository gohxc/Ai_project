...
流式/普通响应切换已放在页面头部，方便在接口测试、聊天测试等页面直接切换。
...
- 真实速度测试：使用每个中转站档案自己的 Key 和模型，只测试当前生成接口，不再重复请求 `/models`。

测速请求会并发执行。真实速度测试默认每个中转站模型跑 5 次，并显示成功次数、最小耗时、平均耗时和最大耗时。

测速结果按成功优先、平均耗时从低到高排序。
